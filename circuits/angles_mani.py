...
def get_angles_from_direction_cosines(dx, dy, dz):
    ...
    try:
        ...
        raise ValueError("Error al calcular ángulos desde cosenos directores.") from e
...
def get_amplitudes_from_angles(theta, phi):
    ...
    try:
        ...
        raise ValueError("Error al calcular amplitudes desde ángulos.") from e
...
def get_direction_cosines_from_angles(theta, phi):
    ...
    try:
        ...
        raise ValueError("Error al calcular cosenos directores desde ángulos.") from e
...
def apply_hadamard(amplitudes):
    ...
    try:
        ...
        raise ValueError("Error al aplicar la puerta Hadamard.") from e
...
def function_of_amplitudes_and_parameter(amplitudes, parameter):
    ...
    try:
        ...
        raise ValueError("Error al calcular función de amplitudes y parámetro.") from e
...
def simulate_measurement(amplitudes, true_parameter):
    ...
    try:
        ...
        raise ValueError("Error al simular la medición.") from e

def probability_density_function(measurement, parameter, quantum_state_amplitudes):
    ...
    try:
        ...
        raise ValueError("Error al calcular la función de densidad de probabilidad.") from e

def likelihood(parameter, measurement, quantum_state_amplitudes):
    """ Calcula la verosimilitud (likelihood) de un parámetro dado una medición. """
    try:
        return probability_density_function(measurement, parameter, quantum_state_amplitudes)
    except Exception as e:
        raise ValueError("Error al calcular la verosimilitud.") from e

def update_bayesian_estimate(prior_mean, prior_variance, measurement, quantum_state_amplitudes, num_samples=1000):
    ...
    try:
        ...
        likelihoods = np.array([likelihood(param, measurement, quantum_state_amplitudes) for param in parameter_samples])
        ...
        raise ValueError("Error al actualizar la estimación bayesiana.") from e
...
def complex_to_real_vector(complex_vector):
    ...
    try:
        ...
        raise ValueError("Error al convertir vector complejo a real.") from e
...
posterior_mean, posterior_variance = update_bayesian_estimate(
    prior_mean, prior_variance, observed_measurement, quantum_state_amplitudes)
